...
pub const Value = struct {
    ...
    pub fn toBigIntAdvanced(
        ...
        switch (val.tag()) {
            ...
            .elem_ptr => {
                const elem_ptr = val.castTag(.elem_ptr).?.data;
                const array_addr = (try elem_ptr.array_ptr.getUnsignedIntAdvanced(mod, sema_kit)).?;
    ...
    /// If the value fits in a u64, return it, otherwise null.
    /// Asserts not undefined.
    pub fn getUnsignedIntAdvanced(val: Value, mod: *const Module, sema_kit: ?Module.WipAnalysis) !?u64 {
        switch (val.tag()) {
            .zero,
            .bool_false,
            .the_only_possible_value, // i0, u0
            ...
            .lazy_align => {
                const ty = val.castTag(.lazy_align).?.data;
                if (sema_kit) |sk| {
                    return (try ty.abiAlignmentAdvanced(mod, .{ .sema_kit = sk })).scalar;
                } else {
            ...
            .lazy_size => {
                const ty = val.castTag(.lazy_size).?.data;
                if (sema_kit) |sk| {
                    return (try ty.abiSizeAdvanced(mod, .{ .sema_kit = sk })).scalar;
                } else {
    ...
    pub fn orderAgainstZeroAdvanced(
        ...
        return switch (lhs.tag()) {
            ...
            .lazy_align => {
                ...
                if (try ty.hasRuntimeBitsAdvanced(mod, false, sema_kit)) {
                    return .gt;
                } else {
                    return .eq;
                }
            ...
            .elem_ptr => {
                ...
                switch (try elem_ptr.array_ptr.orderAgainstZeroAdvanced(mod, sema_kit)) {
                    .lt => unreachable,
                    .gt => return .gt,
                    .eq => {
                        if (elem_ptr.index == 0) {
    ...
    pub fn orderAdvanced(lhs: Value, rhs: Value, mod: *const Module, sema_kit: ?Module.WipAnalysis) !std.math.Order {
        const lhs_tag = lhs.tag();
        const rhs_tag = rhs.tag();
        const lhs_against_zero = try lhs.orderAgainstZeroAdvanced(mod, sema_kit);
        const rhs_against_zero = try rhs.orderAgainstZeroAdvanced(mod, sema_kit);
        ...
        const lhs_bigint = try lhs.toBigIntAdvanced(&lhs_bigint_space, mod, sema_kit);
        const rhs_bigint = try rhs.toBigIntAdvanced(&rhs_bigint_space, mod, sema_kit);
    ...
    pub fn compareHeteroAdvanced(
        ...
        return (try orderAdvanced(lhs, rhs, mod, sema_kit)).compare(op);
    }
    ...
    pub fn compare(lhs: Value, op: std.math.CompareOperator, rhs: Value, ty: Type, mod: *Module) bool {
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            while (i < ty.vectorLen()) : (i += 1) {
                if (!compareScalar(lhs.indexVectorlike(i), op, rhs.indexVectorlike(i), ty.scalarType(mod), mod)) {
                    return false;
                }
            }
            return true;
    ...
    /// Asserts the value is comparable.
    /// Vector results will be reduced with AND.
    pub fn compareWithZero(lhs: Value, op: std.math.CompareOperator, mod: *const Module) bool {
        return compareWithZeroAdvanced(lhs, op, mod, null) catch unreachable;
    }
    ...
    pub fn compareWithZeroAdvanced(
        ...
        switch (lhs.tag()) {
            .repeated => return lhs.castTag(.repeated).?.data.compareWithZeroAdvanced(op, mod, sema_kit),
            .aggregate => {
                for (lhs.castTag(.aggregate).?.data) |elem_val| {
                    if (!(try elem_val.compareWithZeroAdvanced(op, mod, sema_kit))) return false;
                }
                return true;
            },
        ...
        return (try orderAgainstZeroAdvanced(lhs, mod, sema_kit)).compare(op);
    }
    ...
    pub fn eqlAdvanced(
        ...
        return (try orderAdvanced(a, b, mod, sema_kit)).compare(.eq);
    }
    ...
    pub fn intToFloatAdvanced(val: Value, arena: Allocator, int_ty: Type, float_ty: Type, mod: *const Module, sema_kit: ?Module.WipAnalysis) !Value {
        const target = mod.getTarget();
        if (int_ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, int_ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try intToFloatScalar(val.indexVectorlike(i), arena, float_ty.scalarType(mod), target, sema_kit);
            }
    ...
    pub fn intToFloatScalar(val: Value, arena: Allocator, float_ty: Type, mod: *const Module, sema_kit: ?Module.WipAnalysis) !Value {
        const target = mod.getTarget();
        switch (val.tag()) {
            ...
            .lazy_align => {
                const ty = val.castTag(.lazy_align).?.data;
                if (sema_kit) |sk| {
                    return intToFloatInner((try ty.abiAlignmentAdvanced(mod, .{ .sema_kit = sk })).scalar, arena, float_ty, target);
                } else {
    ...
    pub fn intAddSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data) |*scalar, i| {
                scalar.* = try intAddSatScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn intSubSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try intSubSatScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn intMulWithOverflow(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                const of_math_result = try intMulWithOverflowScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
    ...
    pub fn numberMulWrap(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try numberMulWrapScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn intMulSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try intMulSatScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn bitwiseNot(val: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try bitwiseNotScalar(val.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn bitwiseNand(lhs: Value, rhs: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try bitwiseNandScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floatRem(lhs: Value, rhs: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data) |*scalar, i| {
                scalar.* = try floatRemScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floatMod(lhs: Value, rhs: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floatModScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn intTruncBitsAsValue(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try intTruncScalar(val.indexVectorlike(i), allocator, signedness, @intCast(u16, bits.indexVectorlike(i).toUnsignedInt(mod)), target);
            }
            return Value.Tag.aggregate.create(allocator, result_data);
        }
    ...
    pub fn shlWithOverflow(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                const of_math_result = try shlWithOverflowScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), allocator, target);
                overflowed_data[i] = of_math_result.overflowed;
                scalar.* = of_math_result.wrapped_result;
            }
            return OverflowArithmeticResult{
    ...
    pub fn shlSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try shlSatScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn shlTrunc(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try shlTruncScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn shlTruncScalar(lhs: Value, rhs: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
    ...
    pub fn floatNeg(
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floatNegScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floatDiv(
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floatDivScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floatDivFloor(
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floatDivFloorScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floatDivTrunc(
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floatDivTruncScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floatMul(
        ...
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floatMulScalar(lhs.indexVectorlike(i), rhs.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn sqrt(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try sqrtScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn sin(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try sinScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn cos(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try cosScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn tan(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try tanScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn exp(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try expScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn exp2(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try exp2Scalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn log(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try logScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn log2(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try log2Scalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn log10(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try log10Scalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn fabs(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try fabsScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn floor(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try floorScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn ceil(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try ceilScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn round(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try roundScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn trunc(val: Value, float_type: Type, arena: Allocator, mod: *const Module) Allocator.Error!Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data) |*scalar, i| {
                scalar.* = try truncScalar(val.indexVectorlike(i), float_type.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
