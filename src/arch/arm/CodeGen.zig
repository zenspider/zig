...
fn shiftNormal(
    ...
    switch (tag) {
        .shr => return result,
        .shl => switch (lhs_ty.zigTypeTag(mod)) {
            .Vector => return self.fail("TODO ARM binary operations on vectors", .{}),
            .Int => {
                const int_info = lhs_ty.intInfo(mod);
                if (int_info.bits <= 32) {
        ...
        .Pointer => switch (typed_value.ty.ptrSize()) {
            .Slice => {},
                ...
                switch (typed_value.val.tag()) {
                    .int_u64 => {
                        return MCValue{ .immediate = @intCast(u32, typed_value.val.toUnsignedInt(mod)) };
                    },
                    else => {},
                }
            },
        ...
        .Int => {
            ...
            if (info.bits <= ptr_bits) {
                const unsigned = switch (info.signedness) {
                    ...
                    .unsigned => @intCast(u32, typed_value.val.toUnsignedInt(mod)),
                };
